...
static void *func(void *p){
	...
	for(i = yb;i<=ye;i++)
	for(j = xb;j<=xe;j++)
		MessageBox::back.push_back(*(screen::mem+i*screen::xres+j));
	...
	for(i = 0;i<MessageBox::time;i++){
		screen::fill_rect(xb,yb,xe,ye,MESSAGEBOX_BACK_COLOR-i*100);
		screen::draw_string(xb,yb,str,COLOR_TEXT);
		screen::draw_rect(xb,yb,xe,ye,MESSAGEBOX_BORDER_COLOR);
		sleep(1);
	}
	...
	for(i = yb;i<=ye;i++)
	for(j = xb;j<=xe;j++)
		*(screen::mem+i*screen::xres+j) = MessageBox::back[k++];
	MessageBox::back.clear();
}
